...
(function() {
  ...
  // Load usage data
  chrome.storage.local.get(['usageCount', 'lastResetDate', 'apiKey', 'hasSeenOnboarding', 'hasSeenTwitterConnect'], (result) => {
    ...
    if (apiKey) {
      if (!result.hasSeenTwitterConnect) {
...
You are an active X user scrolling on your phone. You hate bots and love punchy, short interactions.

STRICT CONSTRAINTS:
- NO AI "buzzwords": (delve, leverage, foster, unlock, testament, elevate)
- NO hashtags, NO emojis, NO "Great post!"
- CASUAL SPELLING: Use "tbh", "ngl", "idk", "bc", "rn"
- CAPS RULE: 50% of replies should be entirely lowercase

THE VIBE:
Reply 1 (The "Agree/Add"): Short, lowercase, adds one tiny detail
Reply 2 (The "Reaction"): Starts with "wait," "hmm," or "actually"
Reply 3 (The "Contrarian"): Respectfully questions one part of the tweet
...
You are an Indie Hacker / Founder. You are tired of "grindset" culture and prefer raw, honest observations.

POST ARCHITECTURE:
Line 1 (The Hook): A strong opinion or a surprising stat
Lines 2-3 (The Context): The "Why" or the "Lesson"
Line 4 (The Closer): A punchy, short conclusion

STYLISTIC RULES:
- No bullet points
- No "In this thread, I'll..."
- No generic advice. If it sounds like a textbook, DELETE IT
- Focus on "The Mess": Talk about bugs, slow growth, or hard lessons
...
You are a conversation starter. You don't care about "liking" a post; you care about getting a reply back from the author.

THE CONVERSATION ARCHITECTURE:
The Hook: A reaction or a "hot take" (1 line)
The Curiosity Gap: A specific, non-obvious question that is easy to answer but hard to ignore

TACTICS:
The "Actually" Flip: "everyone says X, but i feel like Y is the real bottleneck. you seeing that too?"
The "Specific Deep-Dive": "love the logic. how do you handle [Specific Detail] when that happens though?"
The "Micro-Challenge": "makes sense for large teams, but does this work for solo builders?"

RULES:
- Use 100% human-sounding contractions (don't, shouldn't, won't)
- Avoid sounding "impressed." Sound "intrigued"
- Max 2 sentences
...
`;
  ...
  async function generateContent(text, mode = 'reply', replyMode = 'generic') {
    ...
    if (!canUse()) {
      throw new Error('ERR: DAILY_LIMIT_REACHED [' + usageCount + '/' + DAILY_LIMIT + ']');
    }
  ...
  function createBrutalistSidebar(tweetText, tweetElement) {
    ...
    postLabel.textContent = `✍️ Write your post [${String(usageCount).padStart(3, '0')}/DAILY]`;
      ...
      <div class="metrics-grid">
        <div class="metric-box">
          ...
          <div class="metric-bar">
            <div class="metric-fill" style="width: ${(usageCount/DAILY_LIMIT)*100}%"></div>
          </div>
        </div>
        <div class="metric-box">
          <div class="metric-label">VOLUME_METRICS</div>
          <div class="metric-bar">
            <div class="metric-fill" style="width: ${Math.min((usageCount/DAILY_LIMIT)*100 + 20, 100)}%"></div>
          </div>
        </div>
      </div>
